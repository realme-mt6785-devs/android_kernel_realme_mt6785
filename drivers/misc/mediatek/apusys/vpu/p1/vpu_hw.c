...
int vpu_reg_lock(struct vpu_device *vd, bool boot, unsigned long *flags)
{
	...
	if (ret == 0) {
		...
		pr_err("%s: vpu%d: failed caused by other thread\n",
			__func__, vd->id);
	}
	...
	uint32_t addr, uint32_t size, uint32_t bin)
{
	...
	if (!mva)
		pr_err("%s: vpu%d: iova allcation failed\n", __func__, vd->id);
	...
	uint32_t bin)
{
	...
	if (info->flag & 0x1 /* EXE_SEG */) {
		...
		vi = &dummy_iova;
		pr_warn("%s: vpu%d: unexpected segment: flags: %x\n",
...
retry:
	...
	if (ret) {
		pr_err("%s: vpu%d: \"%s\" was not found\n",
...
int vpu_init_dev_hw(struct platform_device *pdev, struct vpu_device *vd)
{
	...
	if (ret) {
		pr_err("%s: %s: fail to request irq: %d\n",
	...
	if (ret) {
		pr_err("%s: %s: fail to init commands: %d\n",
			__func__, vd->name, ret);
		goto out;
	}
...
out:
	...
	if (ret)
		pr_err("%s: vpu%d: fail to set debug: %d\n",
			__func__, vd->id, ret);
...
out:
	trace_vpu_cmd(vd->id, 0, "", VPU_CMD_SET_FTRACE_LOG,
		atomic_read(&vd->pw_boost), start_t, ret, 0, 0);
	if (ret)
		pr_err("%s: vpu%d: fail to set ftrace: %d\n",
			__func__, vd->id, ret);
