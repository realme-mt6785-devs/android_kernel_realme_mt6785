...
static int get_row_coe_info(void)
{
	...
	offset = fdt_path_offset(fdt, "/odm/light_1");
	if (offset < 0) {
		printk("get als radio fail");
		return -1;
	}

	data = (char *)fdt_getprop(fdt, offset, "als_ratio", &len);
	if (NULL == data) {
		printk("get als radio fail");
		return -1;
	} else {
		row_coe = fdt32_to_cpu(*(int *)data);
		printk("get als radio = %d", row_coe);
	}
...
static int __init oppo_als_cali_data_init(void)
{
	...
	gdata = data;
	gdata->row_coe = get_row_coe_info();
	if (gdata->row_coe == -1) {
		gdata->row_coe = 300;
	}
