...
static inline void mt_i2c_init_hw(struct mt_i2c *i2c)
{
	...
	pr_debug("pcb version=%u\n", pcb_version);
	if (is_project(19151) || is_project(19350)) {
	    pr_debug("19151 and 19350 no need to disable i2c2 and i2c4 inside pull up\n");
	} else if (is_project(19531) && (pcb_version > HW_VERSION__13)) {
	    pr_debug("19531&&pcb_version > HW_VERSION__13 no need to disable i2c2 and i2c4 inside pull up\n");
	    ...
	    val |= ((0 << 30));
	    writel(val, i2c_2_4_pull_base + 0x60);
	    pr_debug("disable i2c2 and i2c4 inside pull up\n");
	}
	#endif
}
